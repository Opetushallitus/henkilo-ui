...
class EmailVerificationErrorContainer extends React.Component<Props> {
    ...
    render() {
        ...
        } else if(virhekoodi === 'TOKEN_VANHENTUNUT_EI_LOYDY') {
            return <VirhePage
                theme="virhe"
                topic="SAHKOPOSTI_VARMENNUS_TOKEN_VANHENTUNUT_EI_LOYDY_OTSIKKO"
                text="SAHKOPOSTI_VARMENNUS_TOKEN_VANHENTUNUT_EI_LOYDY_TEKSTI"
                buttonText="SAHKOPOSTI_VARMENNUS_KIRJAUTUMISSIVULLE"/>;
        } else if(virhekoodi === 'tuntematon') {
    ...
    redirectToFrontPage = async () =>{
        try {
            const url = urls.url('kayttooikeus-service.cas.logintoken.redirectToFrontpage', this.props.loginToken, this.props.locale);
            const redirectUrl = await http.get(url);
            const loginUrl = urls.url('cas.login', redirectUrl);
            window.location.replace(loginUrl);
        } catch (error) {
            this.props.router.push(`/sahkopostivarmistus/virhe/${this.props.locale}/${this.props.loginToken}/tuntematon`);
        }
    }
}
